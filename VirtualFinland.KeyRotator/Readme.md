...
WORKDIR /src
COPY ["VirtualFinland.KeyRotator.csproj", "VirtualFinland.KeyRotator/"]
RUN dotnet restore "VirtualFinland.KeyRotator/VirtualFinland.KeyRotator.csproj"

WORKDIR "/src/VirtualFinland.KeyRotator"
COPY . .
RUN dotnet build "VirtualFinland.KeyRotator.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "VirtualFinland.KeyRotator.csproj" \
...
Execute unit tests
```
cd "VirtualFinland.KeyRotator/test/VirtualFinland.KeyRotator.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "VirtualFinland.KeyRotator/src/VirtualFinland.KeyRotator"
